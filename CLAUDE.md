...
- `POST /v1/didcomm/send` packs and then POSTs to the `deliveryEndpoint`. `src/services/delivery.ts` refuses private networks **twice**: a pre-flight DNS check that names the address (400 `DeliveryRefused`), and a filtered `lookup` inside the undici Agent's connector, which closes DNS rebinding because the address checked is the address dialed. IPv4 is a blocklist (RFC 1918, loopback, link-local, CGNAT, test-nets, multicast); IPv6 is an **allowlist** — global unicast `2000::/3` or refused — so unenumerated ranges (fec0::/10, NAT64, …) fail closed; v4-mapped v6 is judged as its embedded IPv4. Redirects are **not followed** (`redirect: "manual"`) — a 3xx is reported as the recipient's answer, because following one would carry the message to an address the guard never saw, an IP literal not even reaching `lookup`. `ALLOW_PRIVATE_DELIVERY=true` (or `buildServer({ allowPrivateDelivery: true })`, which the send tests use) lifts the network policy only — scheme and URL checks stay
